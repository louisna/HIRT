...
struct PacketOdesc {
    ...
    inline PacketOdesc(const Element *e, WritablePacket *p, int default_ip_p, const IPFlowID *default_ip_flowid, const IP6FlowID *default_ip6_flowid, int minor_version);
    void clear_values()                 { vptr[0] = vptr[1] = 0; }
...
inline PacketOdesc::PacketOdesc(const Element *e_, WritablePacket* p_, int default_ip_p_, const IPFlowID *default_ip_flowid_, const IP6FlowID *default_ip6_flowid_, int minor_version_)
    : p(p_), is_ip(true), is_ip6(false), have_icmp_type(false), have_icmp_code(false),
      have_ip_hl(false), have_tcp_hl(false),
      e(e_), default_ip_p(default_ip_p_), default_ip_flowid(default_ip_flowid_), default_ip6_flowid(default_ip6_flowid_),
