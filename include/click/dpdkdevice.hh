...
public:
...
#else
    inline static bool is_dpdk_packet(Packet* p) {
        return p->buffer_destructor() == DPDKDevice::free_pkt || p->buffer_destructor() == DPDKDevice::free_pkt_empty;
    }
    ...
    inline static void kill_ref(Packet* p); 
    
    static void free_pkt(unsigned char *, size_t, void *pktmbuf);
    static void free_pkt_empty(unsigned char *, size_t, void *pktmbuf);
...
inline void DPDKDevice::kill_ref(Packet* p) {
    struct rte_mbuf* mbuf = (struct rte_mbuf *)((unsigned char*) p->buffer() - sizeof(rte_mbuf));
